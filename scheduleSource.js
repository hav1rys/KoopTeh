...
// \b сразу после кириллицы в JS не срабатывает (\w — только ASCII), поэтому
// здесь и ниже (строка "прав\b") используется (?=\s|$) вместо \b.
const SUBJECT_ICONS = [
  [/(?:^|\W)(?:физ(?:ическ|культур)|физ-?ра)(?=\s|$)/i, '🏃'],
  ...
  [/прав(?=\s|$)|юрид|юриспруд|гражданск/i, '⚖️'],
